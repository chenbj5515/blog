...
而通过React跑scheduler来学习scheduler大多数时候是很蠢的，因为React相关的概念比scheduler本身多多了。

这篇文章帮助我们解决了这个窘境，作者提供了一个模仿react使用scheduler的小demo，我们就找到了很好的角度，可以高效地学习了。
...
[demo源码位置](https://github.com/chenbj5515/blog/blob/main/packages/react-scheduler/src/index.tsx)

sheduler源码在安装后可以在demo所在目录的node_modules中看到，如需要可以在源码中log/debugger然后重新pnpm run dev即可
...
## 接下来做什么？
即使你对React源码没有任何了解也可以读懂这篇文章，因为scheduler是一个独立的包，完全可以给其他库使用。

然而我们的根本目的不是学习scheduler，因为就这么几百行代码，其实也没有太多好学的。我的根本目的是探究React调度的完整流程。

而sheduler这个包仅仅是React调度的核心部分，想要了解react调度的全貌，比如想了解全部内容，还需要了解lane模型以及demo中的work/shedule/perform在React源码到底是怎样的，有机会的话我会在另一篇文章中继续探究。
